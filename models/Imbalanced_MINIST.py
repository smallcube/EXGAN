...
class MNIST(VisionDataset):
    ...
    def __init__(
        ...
        index0 = (self.targets==0)
        index1 = (self.targets==1)
        data0 = self.data[index0]
        data1 = self.data[index1]
        target0 = self.targets[index0]
        target1 = self.targets[index1]

        if self.train:
            count = int(data0.shape[0]/ir+0.5)
        else:
            count = data1.shape[0]
        self.data = torch.cat([data0, data1[0:count]], dim=0)
        self.targets = torch.cat([target0, target1[0:count]], dim=0)
